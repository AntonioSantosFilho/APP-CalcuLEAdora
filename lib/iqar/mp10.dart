...
TextEditingController mp10Controller = TextEditingController();

class Mp10 extends StatefulWidget {
  const Mp10({super.key});

  @override
  State<Mp10> createState() => _Mp10State();
}

class _Mp10State extends State<Mp10> {
  @override
                    ...
                    SingleChildScrollView(
                      child: Column(
                        children: [
                          ...
                          const SizedBox(height: 30),
                          _buildInputField("MP10 (µg/m³)", mp10Controller,
                              calcularmp10, _getFormulaMP10),
                              ...
                              onPressed: () {
                                if (_areAllFieldsFilled()) {
                                  double valor = double.parse(
                                      mp10Controller.text.replaceAll(',', '.'));
                                  double resultado = calcularmp10(valor);
  ...
  bool _areAllFieldsFilled() {
    return mp10Controller.text.isNotEmpty;
  }

  double calcularmp10(double cl) {
    if (cl >= 0 && cl <= 50) {
      return (40 / 50) * cl;
    } else if (cl > 50 && cl <= 100) {
      return ((80 - 41) / (100 - 50)) * (cl - 50) + 41;
    } else if (cl > 100 && cl <= 150) {
      return ((120 - 81) / (150 - 100)) * (cl - 100) + 81;
    } else if (cl > 150 && cl <= 250) {
      return ((200 - 121) / (250 - 150)) * (cl - 150) + 121;
    } else if (cl > 250) {
      return ((cl - 250) * (200 / 100)) + 200;
    } else {
      return double.nan;
    }
  }
  ...
  void _showResultadoDialog(BuildContext context, double resultado) {
    ...
    if (resultado >= 0 && resultado <= 40) {
      ...
      cor = const Color.fromARGB(255, 161, 145, 0);
  ...
  String _getFormulaMP10(double cl) {
    if (cl >= 0 && cl <= 50) {
      return "IQAr = (40 / 50) * concentração";
    } else if (cl > 50 && cl <= 100) {
      return "IQAr = ((80 - 41) / (100 - 50)) * (concentração - 50) + 41";
    } else if (cl > 100 && cl <= 150) {
      return "IQAr = ((120 - 81) / (150 - 100)) * (concentração - 100) + 81";
    } else if (cl > 150 && cl <= 250) {
      return "IQAr = ((200 - 121) / (250 - 150)) * (concentração - 150) + 121";
    } else if (cl > 250) {
      return "IQAr = ((concentração - 250) * (200 / 100)) + 200";
    } else {
